...
<title><s:if test="null != user.id">Edit</s:if><s:else>Add</s:else>
	- S2SMDemo</title>
</head>
<body>
	<h1>
		<s:if test="null != user.id">Edit</s:if>
		<s:else>Add</s:else>
	</h1>
				...
				<td><select name="categoryId">
						<s:iterator value="categoryList" var="item">
							<option value="<s:property value="#item.id"/>"
								<s:if test="user.categoryId == #item.id">selected="selected"</s:if>><s:property
									value="#item.name" />
							</option>
						</s:iterator>
				</select></td>
