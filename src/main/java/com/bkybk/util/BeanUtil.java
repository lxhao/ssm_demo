...
public class BeanUtil extends BeanUtils {
	...
	static {
		...
		dtConverter.setPatterns(new String[] { "yyyy-MM-dd HH:mm:ss",
				"yyyy-MM-dd", "MM/dd/yyyy", "MM/dd/yyyy HH:mm:ss" });
	...
	private static Boolean compareValue(Object oldObj, Object newObj) {
		...
		// 防止旧值为null
		(oldObj == null ? newObj.getClass() : oldObj.getClass())
				.equals(BigDecimal.class) &&
		// 如果旧值为null时给一个0
				(oldObj == null ? new BigDecimal(0) : (BigDecimal) oldObj)
						.compareTo(
						// 新值为null时给一个0
						newObj == null ? new BigDecimal(0)
								: (BigDecimal) newObj) == 0) {
			return true;
			// 属性类型为Timestamp
		} else if ((oldObj == null ? newObj.getClass() : oldObj.getClass())
				.equals(Timestamp.class)
		// 避免出现null.eal的情况
				&& (oldObj == null ? newObj.equals(oldObj) : oldObj
						.equals(newObj))) {
			return true;
		} else {
